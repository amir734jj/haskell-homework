...
-- calculates the list C such that C_l = cos(2*pi*k*l/n)
c :: Double -> Double -> [Double]
c n k = map cos (range 0 (2*pi*k) n)

-- calculates the list s such that s_l = sin(-2*pi*k*l/n)
s :: Double -> Double -> [Double]
s n k = map sin (range 0 (-2*pi*k) n)

-- calculates the tuple X_k for a given list
tuple :: [Double] -> Double -> (Complex Double)
tuple list k =
  let first = sum (zipWith (*) list (c (fromIntegral(length list)) k))
      second = sum (zipWith (*) list (s (fromIntegral(length list)) k))
  in first :+ second

-- dft function
dft :: [Double] -> Vec (Complex Double)
dft [] = mempty
dft list = fmap (tuple list) (Vec (map fromIntegral [0..((length list)-1)]))
