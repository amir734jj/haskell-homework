...
low_pass freq list = (runReader $ ifft $ liftA2 (*) (mask freq (length' list)) ((runReader $ fft list) twiddlelist)) twiddlelist
  where twiddlelist = twiddle (fromIntegral $ length' list) 1
...
makeNoise seed n low high =
      let
          iterator = state $ (randomR (low,high))
          firstValue = runState iterator (mkStdGen seed)
          list = take n $ iterate (\x -> runState iterator (snd x)) firstValue
      in Vec $ map fst list
